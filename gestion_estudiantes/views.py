...
def registro_usuario(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.collection('instructoresManejo').document(user.uid).set({
                'email' : email,
                'uid' : user.uid,
                'rol' : 'Instructor/a de conducción',
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return redirect('listar_estudiantesManejo_manejo')
            else:
                # Error: Analizarlo
...
def dashboard(request):
    ...
    try:
        ...
        doc_ref = db.collection('instructoresManejo').document(uid)
        doc = doc_ref.get()
...
def listar_estudiantesManejo(request):
    """
    ...
    try:
        ...
        docs = db.collection('estudiantesManejo').where('usuario_id', '==', uid).stream()
        for doc in docs:
            estudiante = doc.to_dict()
            estudiante['id'] = doc.id
            estudiantesManejo.append(estudiante)
    except Exception as e:
        messages.error(request, f"Hubo un error al obtener los estudiantes {e}")
    
    return render(request, 'estudiantesManejo/listar.html', {'estudiantes' : estudiantesManejo})

@login_required_firebase # Verifica que el usuario esta loggeado
def anadir_estudianteManejo(request):
    """
    ...
    if (request.method == 'POST'):
        ...
        try:
            db.collection('estudiantesManejo').add({
    ...
    return render(request, 'estudiantesManejo/form.html')

@login_required_firebase # Verifica que el usuario esta loggeado
def eliminar_estudianteManejo(request, estudiante_id):
    """
    DELETE: Eliminar un documento especifico por id
    """
    try:
        db.collection('estudiantesManejo').document(estudiante_id).delete()
...
@login_required_firebase # Verifica que el usuario esta loggeado
def editar_estudianteManejo(request, estudiante_id):
    """
    UPDATE: Recupera los datos del estudiante especifico y actualiza los campos en firebase
    """
    uid = request.session.get('uid')
    estudiante_ref = db.collection('estudiantesManejo').document(estudiante_id)
    ...
    return render(request, 'estudiantesManejo/editar.html', {'estudiante': estudiante_data, 'id': estudiante_id})